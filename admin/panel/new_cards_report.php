...
<head>
	...
	<title>Digital Word - Cards Report</title>
...
<body>
	...
	<div class="content">
		...
		<div class="page-header full-content bg-blue-grey">
			<div class="row">
				<div class="col-sm-6">
					<h1>Unsold Cards<small>&nbsp;Report</small></h1>
				</div><!--.col-->
				<div class="col-sm-6">
					<ol class="breadcrumb">
						...
						<li><a href="#" class="active">Unsold Cards Report</a></li>
					</ol>
				</div><!--.col-->
			</div><!--.row-->
		</div><!--.page-header-->
			...
			<div class="col-md-12">
				<div class="panel">
					<div class="panel-heading">
						<div class="panel-title"><h4>Overall Unsold Cards Report</h4></div>
					</div><!--.panel-heading-->
					<div class="panel-body">
						<?php
						$total_card_query		= "SELECT id FROM cards_collection WHERE status = '0'";	
						$result_total_card		= mysql_query($total_card_query);
						$num_rows 				= mysql_num_rows($result_total_card);						
						$card_records_query		= "SELECT SUM(amount), SUM(market_price), SUM(purchase_price) FROM cards_collection WHERE status = '0'";	
						$result_card_records	= mysql_query($card_records_query);						
						?>
						<table data-toggle="table" data-height="300" data-sort-name="name" data-sort-order="desc" style="height: 30px;">
							<thead>
								<tr>
									<th  data-sortable="true">Total Unsold Cards</th>									
									<th data-sortable="true" data-sorter="priceSorter">Total Retail Price</th>
									<th data-sortable="true" data-sorter="priceSorter">Total Market Price</th>
									<th data-sortable="true" data-sorter="priceSorter">Total Purchase Price</th>
								</tr>								
							</thead>
							<tbody>
							<?php
							while ($row_card_records = mysql_fetch_array($result_card_records))  {
								$total_amount 			= $row_card_records['SUM(amount)'];
								$total_market_price		= $row_card_records['SUM(market_price)'];
								$total_purchase_price	= $row_card_records['SUM(purchase_price)'];																 
							?>
								<tr>
									<td>
										<?=$num_rows?>
					...
					<div class="panel-heading">
						<div class="panel-title"><h3>Company Wise Details</h3></div>
					</div><!--.panel-heading-->
					<?php
					$company_query		= "SELECT DISTINCT company FROM cards_collection";	
					$result_company		= mysql_query($company_query);
						while ($row_company = mysql_fetch_array($result_company))  {
							$company = $row_company['company'];
					?>
							<div class="panel-heading">
								<div class="panel-title"><h4><?=$company?></h4></div>
							</div><!--.panel-heading-->
							<?php
							$company_unsold_card_records_query	= "SELECT id FROM cards_collection WHERE company = '".$company."' AND status = '0'";	
							$result_company_unsold_card_records	= mysql_query($company_unsold_card_records_query);
							$num_rows_unsold_card 				= mysql_num_rows($result_company_unsold_card_records);
							$company_sold_card_records_query	= "SELECT id FROM cards_collection WHERE company = '".$company."' AND status = '1'";	
							$result_company_sold_card_records	= mysql_query($company_sold_card_records_query);
							$num_rows_sold_card 				= mysql_num_rows($result_company_sold_card_records);
							?>	
							<div class="panel-body">
									<table data-toggle="table" data-height="300" data-sort-name="name" data-sort-order="desc" style="height: 30px;">
										<thead>
											<tr>
												<th  data-sortable="true">Total Unsold Cards</th>									
												<th data-sortable="true" data-sorter="priceSorter">Total Sold Cards</th>												
											</tr>								
										</thead>
										<tbody>
											<tr>
												<td>
													<?=$num_rows_unsold_card?>
												</td>									
												<td>
													<?=$num_rows_sold_card?>
												</td>												
