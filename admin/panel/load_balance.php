...
include("db_con.php");
	if ($_POST['form_submitted'] == "form_submitted") {			
			...
			$add_balance_query	= "SELECT balance FROM balance WHERE user_id = '".$_POST['user_id_for_balance']."' ";	
			$result_add_balance	= mysql_query($add_balance_query);
			$num_rows 			= mysql_num_rows($result_add_balance);
				if ($num_rows != 0) {
					$row_add_balance = mysql_fetch_row($result_add_balance);					
					$balance				= $row_add_balance[0];;
					$balance    			= $balance + $_POST['amount'];
					$balance_update_query	= "UPDATE balance SET balance = '".$balance."',last_updated_time_date = '".$today."' WHERE user_id = '".$_POST['user_id_for_balance']."' ";								
					$result_balance_update	= mysql_query($balance_update_query);
					$message_success		= "Balance loaded successfully.";					 
				} else {		
					$insert_query		= "INSERT INTO balance(id, user_id, balance, last_updated_time_date) VALUES(NULL, '".$_POST['user_id_for_balance']."', '".$_POST['amount']."', '".$today."')";								
					$result_insert 		= mysql_query($insert_query);
					$message_success	= "Balance loaded successfully.";
				}
				
				
				if ($_POST['credit'] == "credit") {
					//------------------------ Credit logic ------------------------------------//
					$find_credit_query		= "SELECT credit_amount FROM credit WHERE user_id = '".$_POST['user_id_for_balance']."' ";	
					$result_find_credit		= mysql_query($find_credit_query);
					$num_rows_find_credit 	= mysql_num_rows($result_find_credit);
						if ($num_rows_find_credit == 0) {
							$credit_query	= "INSERT INTO credit(id, user_id, credit_amount, last_updated_time_date) VALUES(NULL, '".$_POST['user_id_for_balance']."', '".$_POST['amount']."', '".$today."')";								
							$result_credit	= mysql_query($credit_query);						
						} else {
							$row_credit 			= mysql_fetch_row($result_find_credit);
							$available_credit 		= $row_credit[0]; 
							$total_credit			= $available_credit + $_POST['amount'];							
							$credit_update_query	= "UPDATE credit SET credit_amount = '".$total_credit."',last_updated_time_date = '".$today."' WHERE user_id = '".$_POST['user_id_for_balance']."' ";								
							$result_credit_update	= mysql_query($credit_update_query);						
						}
					//------------------------ End credit logic ------------------------------------//	
				} else {
					//------------------------ Debit logic ------------------------------------//
					$find_debit_query		= "SELECT credit_amount FROM credit WHERE user_id = '".$_POST['user_id_for_balance']."' ";	
					$result_find_debit		= mysql_query($find_debit_query);
					$num_rows_find_debit 	= mysql_num_rows($result_find_debit);
						if ($num_rows_find_debit != 0) {
							$row_debit 				= mysql_fetch_row($result_find_debit);
							$available_credit		= $row_debit[0];
								if ($_POST['amount'] > $available_credit) {
									$total_credit = 0;
								} else {
									$total_credit = $available_credit - $_POST['amount'];
								}	
							$debit_update_query		= "UPDATE credit SET credit_amount = '".$total_credit."',last_updated_time_date = '".$today."' WHERE user_id = '".$_POST['user_id_for_balance']."' ";								
							$result_debit_update	= mysql_query($debit_update_query); 
						}
...
<head>
	...
	<title>Digital Word - Load Balance</title>
...
#suggest {
	...
	var inputtxt				= document.forms["load_balance_form"]["amount"];
    var numbers 				= /^[0-9]+$/;
    var user_id_load_balance	= $('#user_id_for_balance').val();
    alert(user_id_load_balance);
    	if ($('#user_id_for_balance').val() == "") {
    		alert('Invalid User. Please try again');
      		return false;
    	}	
    	/*
    	if (user_id_load_balance == "") {
      		alert('Invalid User. Please try again');
      		return false;
      	} else {
      		alert(2);
	...
	$('#city').val('');
	$('#country').val('');
	$('#user_id_for_balance').val('');		
		if (option == "email") {
			$('#phone_container').hide();
			$('#email_container').show();
			$('#customer_number_container').hide();
		} else if (option == "phone") {
			$('#phone_container').show();
			$('#email_container').hide();
			$('#customer_number_container').hide();
		} else if (option == "customer_number") {
			$('#phone_container').hide();
			$('#email_container').hide();
			$('#customer_number_container').show();
		}
	}
	...
	function fill(thisValue) {		
		$('#user').val(thisValue);
		var email = $('#user').val(thisValue);		
		setTimeout("$('#suggestions').fadeOut();", 600);	
		$('#amount_container').show();		
	}

	function fill_phone(thisValue) {		
		$('#amount_container').show();
		$.post("autosuggest_phone.php", {queryString: ""+thisValue+""}, function(data) {
			if (data.length >0) {			
				$('#user_id_for_balance').val(data);
				show_user_detail(data);
			}
		});	
	}
	
	function fill_customer_number(thisValue) {		
		$('#amount_container').show();
		$.post("autosuggest_customer_number.php", {queryString: ""+thisValue+""}, function(data) {
			if (data.length >0) {			
				$('#user_id_for_balance').val(data);
				show_user_detail(data);
			}
	...
	function get_user_id() {
		var user = $('#user').val();		
		$.post("get_user_id.php", {queryString: ""+user+""}, function(data) {
			if (data.length >0) {
				$('#user_id_for_balance').val(data);
				show_user_detail(data);					
			}
	...
	function show_user_detail(id) {
		// alert(id);
		$.post("get_user_detail.php", {queryString: ""+id+""}, function(data) {
			if (data.length >0) {					
	...
	function load_user_details(parameter) {
		var user_details = parameter.split("|");
		$('#first_name').val(user_details[1]);
		$('#last_name').val(user_details[2]);
		$('#phone').val(user_details[3]);
		$('#address').val(user_details[4]);
		$('#city').val(user_details[5]);
		$('#country_name').val(user_details[6]);		
	}
	
	function show_mode() {
		$('#mode_container').show();
		$('#buttons').show();
			if (document.getElementById('email_radio').checked) {
				get_user_id();
			}	
	}
...
<body>
	...
	<div class="content">
		...
		<div class="page-header full-content bg-brown">
			<div class="row">
				<div class="col-sm-6">
					<h1>Balance <small>Load balance</small></h1>
				</div><!--.col-->
				<div class="col-sm-6">
					<ol class="breadcrumb">
						...
						<li><a href="load_balance.php" class="active">Load Balance</a></li>
					</ol>
				</div><!--.col-->
			</div><!--.row-->
		</div><!--.page-header-->
			...
			<div class="col-md-12">
				<div class="panel">
					<div class="panel-heading">
						<div class="panel-title"><h4>Balance Details</h4></div>
					</div><!--.panel-heading-->
					<div class="panel-body">

						<form name="load_balance_form" action="load_balance.php" method="post" class="form-horizontal" onsubmit="return validateForm();">							 
							<div class="row">
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Make Selection</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<input type="radio" name="search_criterea" id="email_radio" value="email" checked="" onclick="selection('email');">Search by Email<br />
												<input type="radio" name="search_criterea" id="phone_radio" value="phone" onclick="selection('phone');">Search by Phone<br />
												<input type="radio" name="search_criterea" id="customer_number_radio" value="customer_number" onclick="selection('customer_number');">Search by Customer Number											
											</div>
										</div>
									</div>
								</div>
								<div class="form-group has-success has-feedback" id="email_container">
									<label class="control-label col-md-3" for="inputSuccess2">Select User by Email</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<div id="suggest">
											      <input type="text" size="25" value="" id="user" name="user" onkeyup="suggest(this.value);" onblur="fill();" class="form-control" />     
								...
								<div class="form-group has-success has-feedback" id="phone_container" style="display: none;">
									<label class="control-label col-md-3" for="inputSuccess2">Select User by Phone</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<div id="suggest">
											      <input type="text" size="25" value="" id="phone_search" name="phone_search" onblur="fill_phone(this.value);" class="form-control" />     
											      <div class="suggestionsBox" id="suggestions" style="display: none;"> <img src="arrow.png" style="position: relative; top: -12px; left: 30px;" alt="upArrow" />
											        <div class="suggestionList" id="suggestionsList"> &nbsp; </div>
											      </div>											      
											   </div>											
								...
								<div class="form-group has-success has-feedback" id="customer_number_container" style="display: none;">
									<label class="control-label col-md-3" for="inputSuccess2">Select User by Customer Numebr</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<div id="suggest">
											      <input type="text" size="25" value="" id="customer_number_search" name="customer_number_search" onblur="fill_customer_number(this.value);" class="form-control"/>     
											      <div class="suggestionsBox" id="suggestions" style="display: none;"> <img src="arrow.png" style="position: relative; top: -12px; left: 30px;" alt="upArrow" />
											        <div class="suggestionList" id="suggestionsList"> &nbsp; </div>
											      </div>											      
											   </div>											
								...
								<input type="text" name="user_id_for_balance" id="user_id_for_balance">
								<div class="form-group has-success has-feedback" style="display: none;" id="amount_container">
									<label class="control-label col-md-3" for="inputSuccess2">Amount (SEK)</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<input type="text" class="form-control" id="amount" name="amount" required="" onclick="show_mode();">
								...
								<div class="form-group has-success has-feedback" style="display: none;" id="mode_container">
									<label class="control-label col-md-3" for="inputSuccess2">Select Mode</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper" >
												<select name="credit" id="credit" class="selecter" required="required" >
								...
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Phone</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
								...
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Address</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
								...
								<!--
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Select User</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
											<?php											
											$users_query	= "SELECT * FROM users";	
											$result_users	= mysql_query($users_query);						
											?>	
												<select name="user" id="user" class="selecter" required="required">
													<option value="" selected="selected">Select One</option>
													<?php
														while ($result = mysql_fetch_array($result_users)) {														    
							...
							<div class="form-actions" style="display: none;" id="buttons">
								<div class="row">
