...
return $exts; 
	...
	if ($_POST['form_submitted'] == "form_submitted") {					
		if ($_FILES["card_image"]["name"] != "") {
			...
			if(count($_FILES['card_image']['name'])) {
				$counter = 0;		
					foreach ($_FILES['card_image']['name'] as $file) {					    	
					    // echo $_FILES['card_image']['name'][$counter];
						// echo $file;
						// echo "<br>";
						$target_dir 	= "vrt2v1/";
						$target_file 	= $target_dir . basename($_FILES["card_image"]["name"][$counter]);		
						$uploadOk 		= 1;
						$imageFileType 	= pathinfo($target_file,PATHINFO_EXTENSION);
						// Check if image file is a actual image or fake image
					    $check 			= getimagesize($_FILES["card_image"]["tmp_name"][$counter]);
						    if ($check !== false) {
							...
							// Check file size
							if ($_FILES["card_image"]["size"][$counter] > 500000) {
							...
							// if everything is ok, try to upload file
								...
								$ext = findexts($_FILES['card_image']['name'][$counter]) ;
								...
								$company_retail_price 		= $_POST['company'];
								$company_retail_price_array = explode(":", $company_retail_price);
								$company_name				= $company_retail_price_array[0];								   
									if (move_uploaded_file($_FILES['card_image']['tmp_name'][$counter], $target)) {											
										$insert_query		= "INSERT INTO cards_collection(id, card_image, amount, market_price, purchase_price, company, type, status, time_date) VALUES (NULL, '".$target."', '".$_POST['amount']."', '".$_POST['market_price']."', '".$_POST['purchase_price']."', '".$company_name."', 'Calling Card', '0', '".$today."')";						
										$result_insert 		= mysql_query($insert_query);
											if (!$result_insert) {
											    die('Invalid query: ' . mysql_error());
											}
										$message_success 	= "New card has been entered into system";
								    } else {
...
<head>
	...
	<title>Digital Word - Upload New Card</title>
	...
	var inputtxt			= document.forms["new_card_form"]["amount"];
	var market_price		= document.forms["new_card_form"]["market_price"];
	var purchase_price		= document.forms["new_card_form"]["purchase_price"];	  
    var card_image			= document.forms["new_card_form"]["card_image"].value;
    var card_image_empty 	= card_image.length;
    // alert(card_image_empty);
    var numbers 	= /^[0-9]+$/;    
    	...
    	if (inputtxt.value.match(numbers)) {
      		...
      		document.new_card_form.amount.focus();  
      		return false;  
      	}   
      	if (market_price.value.match(numbers)) {      		
      		// return true;  
      	} else {      		
      		alert('Please input numeric characters only');  
      		document.new_card_form.market_price.focus();  
      		return false;  
      	}   
      	if (purchase_price.value.match(numbers)) {
      		// return true;  
      	} else {  
      		alert('Please input numeric characters only');  
      		document.new_card_form.purchase_price.focus();  
      		return false;  
      	}		        	  
    }   
	...
	var input = document.getElementById("card_image");
	...
	function set_price(param) {		
		var company_retail_price_array = param.split(":");		
		$('#amount').val(company_retail_price_array[1])		 
	}
...
<body>
	...
	<div class="content">
		...
		<div class="page-header full-content bg-brown">
			<div class="row">
				<div class="col-sm-6">
					<h1>New Card <small>Upload new card</small></h1>
				</div><!--.col-->
				<div class="col-sm-6">
					<ol class="breadcrumb">
						...
						<li><a href="upload_new_card.php" class="active">New card</a></li>
					</ol>
				</div><!--.col-->
			</div><!--.row-->
		</div><!--.page-header-->
			...
			<div class="col-md-12">
				<div class="panel">
					<div class="panel-heading">
						<div class="panel-title"><h4>New Card Details</h4></div>
					</div><!--.panel-heading-->
					<div class="panel-body">

						<form name="new_card_form" action="upload_new_card.php" method="post" class="form-horizontal" enctype="multipart/form-data" onsubmit="return validateForm();">							 
							<div class="row">								
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Company</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<?php											
											$users_query	= "SELECT company_name, retail_price FROM companies";	
											$result_users	= mysql_query($users_query);						
											?>	
												<select name="company" id="company" class="selecter" required="required" onchange="set_price(this.value);">
													<option value="" selected="selected">Select One</option>
													<?php
														while ($result = mysql_fetch_array($result_users)) {
															$company_retail_price = $result['company_name'].":".$result['retail_price']; 														    
														    echo "<option value='" . $company_retail_price . "' >" . $result['company_name'] . "</option>";
														}
													?>
												</select>
											</div>
								...
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Market Price (SEK)</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<input type="text" class="form-control" id="market_price" name="market_price" required="">
												<span class="ion-android-done tooltips form-control-feedback" data-toggle="tooltip" data-placement="top" title="Success tooltip"></span>
											</div>
										</div>
									</div>
								...
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Purchase Price (SEK)</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
								...
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2"><font style="color: gray;">How to upload multiple cards</font></label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<img src="images/multiple-file-upload.jpg" height="250">
												<br />
												<font style="color: gray;">(Select multiple cards with Ctrl key like above image)</font>													
											</div>
										</div>
									</div>
								</div>
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Upload Card Image</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
												<input type="file" class="form-control" id="card_image" name="card_image[]" multiple="" onChange="makeFileList();">
												<span class="ion-android-done tooltips form-control-feedback" data-toggle="tooltip" data-placement="top" title="Success tooltip"></span>
											</div>
										</div>
									</div>
								...
								<div class="form-group has-success has-feedback">
									<label class="control-label col-md-3" for="inputSuccess2">Files You Selected:</label>
									<div class="col-md-5">
										<div class="inputer">
											<div class="input-wrapper">
